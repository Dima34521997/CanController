...
public interface IApiCanController
{
    ...
    void CreatePDO(byte Node, ushort Index, byte Subindex)
    => CANOpenDll.AddNodeObjectToDictionary(Node, Index, Subindex, (ushort)Defines.CAN_DEFTYPE_INTEGER32);


    /// <summary>
    /// Создает индекс в виртуальном ОС для записи PDO в формате XXXXYYZZ,
    /// где XXXX - индекс,
    ///       YY - cубиндекс
    ///       ZZ - размер\тип данных в байтах
    ///       
    /// Пример: 20hex = 32dec => int/uint
    /// 
    /// ZZ=20 используется по умолчанию.
    /// 
    /// </summary>
    /// <param name="Index">Индекс</param>
    /// <param name="Subindex">Субиндекс</param>
    /// <returns></returns>
    uint VirtualIndexBuilder(ushort Index, byte Subindex);
    ...
    /// <param name="Index">Индекс</param>
    /// <param name="Subindex">Субиндекс</param>
    /// <param name="PDONumber">Номер PDO (от 1 до 4)</param>
    void BoundPDO(byte Node, ushort Index, byte Subindex, byte PDONumber = 1);
    ...
    /// <param name="PDOData">Отправляемые через PDO данные</param>
    /// <returns>Код-результат выполнения</returns>
    int ReadPDO(byte Node, ushort Index, byte SubIndex, ref byte UPD, ref int PDOData);
